...
char path[PATH_MAX];
int traverse(const char *dir, const char *name_dir, int len_dir)
{
    DIR *d = opendir(dir);
    if (d == NULL) {
        return 0;
    ...
    while ((dd = readdir(d))) {
        if (strcmp(dd->d_name, ".") && strcmp(dd->d_name, "..")) {
            int slen = snprintf(&path[len_dir], sizeof(path) - len_dir, "/%s", dd->d_name);
            if (slen + len_dir + 1 <= sizeof(path)) {
                struct stat info;
    ...
    for (size_t i = 0; i < count; ++i) {
        int slen = snprintf(&path[len_dir], sizeof(path) - len_dir, "/%s", v[i]);
        if (traverse(path, v[i], slen + len_dir)){
            return 1;
        }
        free(v[i]);
    }
